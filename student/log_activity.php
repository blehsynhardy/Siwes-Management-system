				...
				LEFT JOIN department ON department.department_id = supervisor_department_student.department_id LEFT JOIN supervisor ON supervisor.supervisor_id = supervisor_department_student.supervisor_id")or die(mysqli_error($con));
                        ...
                        <div id="block_bg" class="block">
							...
							<?php 	$query = mysqli_query($con,"select * FROM files where department_id = '$department_id' and student_id= $session_id order by fdatein DESC ")or die(mysqli_error($con));
							...
							Check All <input type="checkbox"  name="selectAll" id="checkAll" />
								...
								<?php
									$query = mysqli_query($con,"select * FROM files where department_id = '$department_id' and student_id= $session_id order by fdatein DESC ")or die(mysqli_error($con));
									$count = mysqli_num_rows($query);	
								if ($count == '0'){ ?>
								...
								?>
									...
									?>                              
										...
										<td width="30">
                                         ...
                                         <td width="30">
										 <a  data-placement="bottom" title="Download" id="<?php echo $id; ?>Download" href="../admin/<?php echo $row['floc']; ?>"><i class="icon-download icon-large"></i></a>
										 </td>            
...
0				                            		<option value="Week Seven">Week One</option>
				                            		...
				                            		<option value="Week Forteen">Week Forteen</option>
